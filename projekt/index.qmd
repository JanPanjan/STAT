...
Za delo s podatki sem uporabljal pakete `tidyr`, `purr` za čiščenje in preoblikovanje podatkov, `dplyr`, ki je ustvarjen za delo s podatkovnimi okvirji ter `ggplot2` in `gridExtra` za kvalitetne vizualne predstavitve *distiliranih* podatkov.
...
Porazdelitvi lahko primerjamo s histogrami vizualno ali s tako imenovanim **Q-Q (kvantil-kvantil) plot**, ki primerja kvantile naših podatkov s kvantili teoretične normalne porazdelitve. Če se točke na Q-Q grafu ujemajo z diagonalno premico, to nakazuje na to, da so podatki normalno porazdeljeni.
...
#| column: page
#| code-fold: true
#| code-summary: Q-Q plots za vseh 12 spremenljivk
df |> 
  pivot_longer(cols = everything(), names_to = "spremenljivka") |> 
  ggplot(aes(sample = value)) +
  stat_qq() +
  stat_qq_line() +
  facet_wrap(~ spremenljivka, scales = "free") +
  labs(
    title = "Q-Q plots za vseh 12 spremenljivk",
